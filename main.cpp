...
void check_empty() {
    ...
    Stl<std::string> empty;
    Stl<std::string> s2{"opa"};
    s2 = empty;
    if (s2.size()) fail("incorrect size");
    Stl<std::string> s3(s2);
...
void check_iterators() {
    ...
    Stl<std::pair<int, int>> s{{-3, 5}, {5, 5}, {-4, 1}, {-4, 4}, {0, 1}, {3, 0}};
    if (s.begin()->second != 1 || (++s.begin())->first != -4) fail("wrong begin()");
    Stl<std::pair<int, int>>::iterator cur = s.end();
    Stl<int> small{1};
    Stl<int>::iterator it;
    it = small.begin();
...
void check_operator_less() {
    ...
    Stl<StrangeInt> s{-5, -3, -6, 13, 7, 1000, 963};
...
void check_destructor() {
    ...
    StrangeInt::init();
    {
        Stl<StrangeInt> s{5, 4, 3, 2, 1, 0};
        if (s.size() != 6) fail("wrong size");
    }
    if (StrangeInt::counter) fail("wrong destructor (or constructors)");
    {
        Stl<StrangeInt> s{-3, 3, -2, 2, -1, 1};
        Stl<StrangeInt> s1(s);
        s1.insert(0);
        Stl<StrangeInt> s2(s1);
...
void check_erase() {
    ...
    Stl<std::string> s{"abacaba", "hello", "p"};
    s.erase("miss");
    s.erase("hello");
